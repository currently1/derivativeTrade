...
double asianPrice::AsianCurranApprox(optionModel::OptionType cpFlag, double S, double SA, double X, double t1, double T, double n, double m, double r, double b, double v){
	double dt, my, myi;
	double vxi, vi, vx;
	double Km, sum1, sum2;
	double ti, EA;
	...
	double price;
	z = 1;
	if (cpFlag == optionModel::OptionType::PUT) z = -1;
	dt = (T - t1) / (n - 1);
	if (b == 0){
		EA = S;
	}else{
	...
	if (m > 0){
		if (SA > n / m *X){
			if (cpFlag == optionModel::OptionType::PUT){
				price = 0;
			}
			else{
				SA = SA * m / n + EA * (n - m) / n;
...
double asianPrice::GBlackScholes(optionModel::OptionType cpFlag, double S, double X, double T, double r, double b, double v){
	double d1, d2;
	double price;
	d1 = (log(S / X) + (b + v*v / 2.0) * T) / (v * sqrt(T));
	d2 = d1 - v *sqrt(T);
	if (cpFlag == optionModel::OptionType::CALL){
...
double asianPrice::CND2(double x){
	double L, k;
	double result;
	if (x == 0)
	{
...
double asianPrice::delta(optionModel::OptionType cpFlag, double S, double SA, double X, double t1, double T, double n, double m, double r, double b, double v){
	double greeks, p1,p2;
...
double asianPrice::vega(optionModel::OptionType cpFlag, double S, double SA, double X, double t1, double T, double n, double m, double r, double b, double v){
	double greeks, p1,p2;
...
double asianPrice::gamma(optionModel::OptionType cpFlag, double S, double SA, double X, double t1, double T, double n, double m, double r, double b, double v){
...
double asianPrice::rho(optionModel::OptionType cpFlag, double S, double SA, double X, double t1, double T, double n, double m, double r, double b, double v){
	double greeks, p1,p2;
...
double asianPrice::theta(optionModel::OptionType cpFlag, double S, double SA, double X, double t1, double T, double n, double m, double r, double b, double v){
	double greeks, p1,p2;
	p1 = AsianCurranApprox(cpFlag, S, SA, X, t1, T, n, m, r,  b, v);
	p2 = AsianCurranApprox(cpFlag, S, SA, X, t1, T - 1.0/365.0, n, m, r,  b, v);
	greeks = (p2 - p1) / (1.0/ 365);
	return greeks;
}